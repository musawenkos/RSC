...
namespace RoadSignCapture.Web.Pages.Users
{
    public class EditModel : PageModel
    {
        ...
        public EditModel(RSCDbContext context, IUserService userService, IRoleService roleService, IUserRole userRole, ILogger<EditModel> logger, UserHandler userHandler)
        {
            _context = context;
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            try
            {
                var result = await _userHandler.EditHandleAsync(Command,Command.Users!.Email);

                if (!result.Success)
                {
                    ModelState.AddModelError(string.Empty, result.Error ?? "An error occurred while updating the user.");
